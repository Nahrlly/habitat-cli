...
export async function runAutonomyCycle(input: AutonomyRunInput): Promise<AutonomyRunResult> {
  ...
  const candidates: AutonomyAction[] = snapshot.eva.deployedHumanId ? [{ type: "collect", quantityKg: 1 }, { type: "move", x: snapshot.eva.x + 1, y: snapshot.eva.y }, { type: "noop" }] : snapshot.humans.filter((human) => human.status === "idle").map((human) => ({ type: "deploy", humanId: human.id }));
...
async function loadSnapshot(api: ReturnType<typeof createApiClient>): Promise<AutonomySnapshot> {
  const [humansResponse, evaResponse, boundsResponse] = await Promise.all([
    ...
    api.getJson<AutonomySnapshot["bounds"]>("/world/sectors/current").catch(() => null),
  ]);
  const eva = evaResponse.eva;
  return { registered: true, humans: humansResponse.humans, eva: { deployedHumanId: eva.deployedHumanId, x: eva.x, y: eva.y, carriedKg: eva.carriedResources.reduce((sum, resource) => sum + resource.quantityKg, 0), capacityKg: eva.maxCarryingCapacityKg, exhausted: eva.exhausted }, bounds: boundsResponse };
}
